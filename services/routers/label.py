...
def create_label(project_id: int,
  ...
  background_tasks.add_task(one_iter,
  ...
  label_result_df = mk.from_pandas(label_data)
  ...
  print('End training')
  # background_tasks.add_task(predict_pipeline, data_predict=no_label_data, model_id=model_uuid)
...
def update_label_result(label_result_id: int,
  ...
  sql = select(label_result.c.id,
               ...
               label_result.c.iteration).where(label_result.c.id == label_result_id)
  ...
  mk.from_pandas(label_full).write(os.path.join(label_base_path, f'{label_res[3]}.mk'))
