...
data = pd.read_csv("clean_data.csv", encoding='unicode_escape')[:100]
...
def train(data: Union[pd.DataFrame, dict, list, torch.Tensor],
    ...
    if os.path.exists(model_output_path):
        model = AutoModelForTokenClassification.from_pretrained(model_output_path, num_labels=len(id2label),
                                                   id2label=id2label,
                                                   label2id=label2id)
        
    else:
        model = AutoModelForTokenClassification.from_pretrained('bert-base-uncased', num_labels=len(id2label),
                                                   id2label=id2label,
                                                   label2id=label2id)
...
if __name__ == '__main__':
    #train(data, 10, 1, 'model/')

    pre = predict(['I am John from London', 'Today is thursday'], 'model/')
    for i, j in pre:
        print(i, j)
